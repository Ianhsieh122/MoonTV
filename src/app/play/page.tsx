...
function PlayPageClient() {
  ...
  // 保存優選時的測速結果，避免EpisodeSelector重覆測速
  ...
  const handleSourceChange = async (
    ...
    try {
      ...
      // 如果仍然是同一集數且播放進度有效，則在播放器就緒後恢覆到原始進度
      if (targetIndex !== currentEpisodeIndex) {
        ...
        customType: {
          m3u8: function (video: HTMLVideoElement, url: string) {
            ...
            const hls = new Hls({
              ...
              backBufferLength: 30, // 僅保留 30s 已播放內容，避免內存占用
              ...
              if (data.fatal) {
                switch (data.type) {
                  case Hls.ErrorTypes.NETWORK_ERROR:
                    console.log('網絡錯誤，嘗試恢覆...');
                    hls.startLoad();
                    break;
                  case Hls.ErrorTypes.MEDIA_ERROR:
                    console.log('媒體錯誤，嘗試恢覆...');
      ...
      artPlayerRef.current.on('video:canplay', () => {
        ...
        if (resumeTimeRef.current && resumeTimeRef.current > 0) {
          try {
            ...
            console.log('成功恢覆播放進度到:', resumeTimeRef.current);
          } catch (err) {
            console.warn('恢覆播放進度失敗:', err);
          }
        }
